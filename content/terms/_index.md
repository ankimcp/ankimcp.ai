...
- **Hosted Anki runs a copy of your Anki on our servers** — it may change, and deleting an instance is permanent. Keep your own backups.
...
- **Hosted Anki** — the part of the SaaS Service that runs a copy of the Anki desktop application on AnkiMCP's cloud servers, reachable both by LLM clients over MCP and by you through an in-browser remote desktop
...
- The Hosted Anki feature will remain available in its current form, or that a hosted instance will be reachable without delay (hosted instances sleep when idle and take time to wake)
...
- **Pro tier:** includes everything in the Plus tier, raises the monthly text-to-speech allowance (currently 3,000 credits), and adds the **Hosted Anki** feature (see below) for a recurring fee (currently $15 per month). The Pro tier's features, availability, and pricing may change; material changes are announced as described in Section 12 (Changes to Terms).
...
Hosted Anki is a feature available on the Pro tier. It runs a copy of the Anki desktop application on our cloud servers, which you can reach both through LLM clients over MCP and through an in-browser remote desktop. The following additional terms apply when you use Hosted Anki.

- **Best-effort availability.** Hosted Anki is offered on a best-effort basis. We may change, limit, or discontinue it, and availability, performance, and features are not guaranteed.
...
- When you delete a hosted instance, the stored collection and media for that instance are **permanently removed** and cannot be recovered. See "Hosted Anki" above regarding backups.
